...
use crate::models::{
    ...
    ClipboardJob, ClipboardJobKind, ClipboardJobStatus, EntryStatus, ImageIngestJobDraft,
    JobBacklog, JobCleanupRecord,
};
...
pub struct EntryJobCleanup {
    ...
    pub active_jobs: Vec<JobCleanupRecord>,
}
...
impl Database {
    ...
    fn image_ingest_backlog_on(conn: &Connection) -> Result<JobBacklog, String> {
        conn.query_row(
    ...
    fn active_job_cleanup_for_entries_on(
        conn: &Connection,
        ids: &[String],
    ) -> Result<Vec<JobCleanupRecord>, String> {
        if ids.is_empty() {
            return Ok(Vec::new());
        }
    ...
    pub fn image_ingest_backlog(&self) -> Result<JobBacklog, String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
    ...
    pub fn cleanup_terminal_jobs(&self) -> Result<usize, String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        conn.execute(
    ...
    pub fn claim_next_queued_job(&self) -> Result<Option<ClipboardJob>, String> {
        let mut conn = self.conn.lock().map_err(|e| e.to_string())?;
        let tx = conn.transaction().map_err(|e| e.to_string())?;
        let job_id = tx
            .query_row(
    ...
    pub fn requeue_running_job(&self, job_id: &str, error: &str) -> Result<bool, String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        let updated = conn
            .execute(
                "UPDATE clipboard_jobs
    ...
    pub fn fail_running_job_and_delete_pending_entry(
        ...
        let active_jobs = vec![JobCleanupRecord {
